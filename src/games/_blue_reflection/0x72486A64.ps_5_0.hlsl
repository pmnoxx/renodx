...
// ============================================================================
// Blue Reflection Post-Processing Pixel Shader
// Decompiled and annotated for clarity and maintainability
//
// Sections and comments follow Unity/HLSL best practices and project conventions
// ============================================================================
//#include "./shared.h"

// ---- Created with 3Dmigoto v1.4.1 on Fri Jun 27 10:40:43 2025

// ============================================================================
// Resource Bindings & Constants
// ============================================================================
  ...
  out float4 o0 : SV_Target0)
{
  ...
  // --------------------------------------------------------------------------
  // FXAA Edge Detection & Anti-Aliasing
  // --------------------------------------------------------------------------
  // Sample scene at various offsets for edge detection
  ...
  // Compute edge strengths and thresholds
  ...
  // Sample scene at subpixel offsets for anti-aliasing
  ...
  // --------------------------------------------------------------------------
  // Depth of Field (DOF) and Blur Handling
  // --------------------------------------------------------------------------
  ...
  // --------------------------------------------------------------------------
  // Post-Processing Effects (Bloom, Star, Flare, Light Shaft)
  // --------------------------------------------------------------------------
  r0.xyzw = smplEffectScene_Tex.Sample(smplEffectScene_s, v1.xy).xyzw;
  r0.xyz = r2.xyz * r0.www + r0.xyz;
  r0.xyz = max(float3(0,0,0), r0.xyz);
  // Additive post-processing effects
  r1.xzw = smplBloom_Tex.Sample(smplBloom_s, v1.xy).xyz;
  r0.xyz = r1.xzw * fBloomWeight + r0.xyz;
  ...
  // --------------------------------------------------------------------------
  // Color Grading, Saturation, and Spot Effects
  // --------------------------------------------------------------------------
  r1.xzw = vColorScale.xyz * r0.xyz;
  r0.w = dot(r1.xzw, float3(0.298909992,0.586610019,0.114480004)); // Luminance
  r0.xyz = r0.xyz * vColorScale.xyz + -r0.www;
  r0.xyz = vSaturationScale.xyz * r0.xyz + r0.www;
  // Spot effect (vignette/limb darkening)
  ...
  r0.xyz = r0.xyz * r0.www + r1.xzw;
    ...
    // --------------------------------------------------------------------------
    // Simulate HDR and apply tone curve
/*
    float3 simulateHDRParamsOverride = (3.f, 3.f, 3.f);

    r1.xzw = r0.xyz * simulateHDRParamsOverride.xxx + float3(1,1,1);
    ...
    r0.xyz = simulateHDRParamsOverride.xxx * r0.xyz;
    r0.xyz = r1.yyy ? r1.xzw : r0.xyz;
*/
  r0.xyz *= RENODX_INTERMEDIATE_SCALING;
  // Gamma correction
