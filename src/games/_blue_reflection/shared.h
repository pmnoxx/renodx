...
#define RENODX_RENO_DRT_TONE_MAP_METHOD        renodx::tonemap::renodrt::config::
#include "../../shaders/renodx.hlsl"

// Utility to fix NaNs in HLSL
inline float fix_nans(float v) { return isnan(v) ? 0.0f : v; }
inline float2 fix_nans(float2 v) { return float2(fix_nans(v.x), fix_nans(v.y)); }
inline float3 fix_nans(float3 v) { return float3(fix_nans(v.x), fix_nans(v.y), fix_nans(v.z)); }
inline float4 fix_nans(float4 v) { return float4(fix_nans(v.x), fix_nans(v.y), fix_nans(v.z), fix_nans(v.w)); }
...
#define DEBUG_MARK(v1) if (v1.x < 0.001f && abs(v1.y - 0.5f) < 0.05f) { o0.w = 1.f; o0.xyz = float3(0.f, 2.f, 0.f); return; }

#else

#define DEBUG_MARK(v1)
