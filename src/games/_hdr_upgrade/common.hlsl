...
float ReinhardScalable(float color, float channel_max = 1.f, float channel_min = 0.f, float gray_in = 0.18f,
                       float gray_out = 0.18f) {
...
float4 debug_mode(float4 color, float2 pos, float shift_y = 0.f) {
    ...
    if (pos.x >= box.x && pos.x <= box.x + dim.x && pos.y >= box.y && pos.y <= box.y + dim.y) {
...
float3 HueCorrectAP1(float3 incorrect_color_ap1, float3 correct_color_ap1, float hue_correct_strength = 0.5f) {
    ...
    float3 corrected_color_bt709 =
        renodx::color::correct::Hue(incorrect_color_bt709, correct_color_bt709, hue_correct_strength, 2u);
...
float3 acesTonemap(float3 bt709, float peak_nits = 203.f, float diffuse_white_nits = 203.f) {
    ...
    float3 tonemapped_ap1 =
        renodx::tonemap::aces::ODT(untonemapped_rrt_ap1, aces_min * 48.f, aces_max * 48.f, renodx::color::IDENTITY_MAT)
        / 48.f;
...
float3 ApplyReverseReinhard(float3 color, float scene_type = SCENE_TYPE_UNKNOWN) {
    ...
    if (RENODX_PERCEPTUAL_BOOST_MODE == 0.f) {
        ...
        if (xyz.y > 0.0000001f) {
            float3 newXYZ =
                sign(xyz)
                * renodx::color::pq::Decode(
                    renodx::color::pq::Encode(abs(xyz), 10000.f) * (1.f + RENODX_PERCEPTUAL_BOOST_XYPQ_PARAM), 10000.f);
            new_grey =
                renodx::color::pq::Decode(
                    renodx::color::pq::Encode(new_grey, 10000.f) * (1.f + RENODX_PERCEPTUAL_BOOST_XYPQ_PARAM), 10000.f)
                    .r;
            xyz = lerp(xyz * (newXYZ.y / xyz.y), newXYZ.xyz, RENODX_PERCEPTUAL_BOOST_XYPQ_COLOR);
        }
        color.xyz = lerp(color.xyz, renodx::color::bt709::from::XYZ(xyz) * (mid_point / new_grey),
                         RENODX_PERCEPTUAL_BOOST_XYPQ_STRENGTH * boost_strength);
    ...
    if (RENODX_COLOR_GRADE_STRENGTH == 0) {
        ...
        if (RENODX_COLOR_GRADE_PER_CHANNEL_BLOWOUT_RESTORATION != 0.f
            ...
            graded_sdr_color = renodx::draw::ApplyPerChannelCorrection(
                untonemapped, graded_sdr_color, RENODX_COLOR_GRADE_PER_CHANNEL_BLOWOUT_RESTORATION,
                RENODX_COLOR_GRADE_PER_CHANNEL_HUE_CORRECTION, RENODX_COLOR_GRADE_PER_CHANNEL_CHROMINANCE_CORRECTION);
        }

        return renodx::tonemap::UpgradeToneMap(untonemapped, neutral_sdr_color, graded_sdr_color,
...
float3 RestoreHighlightSaturation(float3 color) {
    if (RENODX_TONE_MAP_TYPE != 0.f && DISPLAY_MAP_TYPE != 0.f) {
        ...
        if (DISPLAY_MAP_TYPE == 1.f) {  // Dice
            ...
            float3 frostbite_color =
                renodx::tonemap::frostbite::BT709(color, frostbite_peak, frostbite_shoulder, frostbite_saturation);
...
float3 ApplyExposureContrastFlareHighlightsShadowsByLuminance(float3 untonemapped, float y,
...
float3 ApplySaturationBlowoutHueCorrectionHighlightSaturation(float3 tonemapped, float3 hue_reference_color, float y,
    ...
    if (config.saturation != 1.f || config.dechroma != 0.f || config.hue_correction_strength != 0.f
        || config.blowout != 0.f) {
...
float3 ApplyACES(float3 untonemapped_ap1) {
    ...
    untonemapped_ap1 =
        ApplyExposureContrastFlareHighlightsShadowsByLuminance(untonemapped_ap1, untonemapped_lum, cg_config);
    float3 tonemapped_ap1 =
        renodx::tonemap::aces::ODT(untonemapped_ap1, aces_min * 48.f, aces_max * 48.f, renodx::color::IDENTITY_MAT)
        / 48.f;
    float3 tonemapped_bt709 = renodx::color::bt709::from::AP1(tonemapped_ap1);
    tonemapped_bt709 = ApplySaturationBlowoutHueCorrectionHighlightSaturation(
        tonemapped_bt709, renodx::color::bt709::from::AP1(untonemapped_ap1), untonemapped_lum, cg_config);
...
/*   renodx::color::grade::Config cg_config = renodx::color::grade::config::Create();
  ...
  untonemapped = ApplyExposureContrastFlareHighlightsShadowsByLuminance(untonemapped, untonemapped_lum, cg_config);
  float3 tonemapped = renodx::tonemap::ExponentialRolloff(untonemapped);
  tonemapped = ApplySaturationBlowoutHueCorrectionHighlightSaturation(tonemapped, untonemapped, untonemapped_lum,
  cg_config);
  */
float3 applyUserTonemap(float3 untonemapped, float userToneMapType) {
    ...
    if (userToneMapType == 7.f) {  // ACES
        float3 outputColorAp1 = renodx::color::ap1::from::BT709(outputColor);
        outputColor = ApplyACES(outputColorAp1);
        return outputColor;
    }

    if (userToneMapType != 0) {  // UserColorGrading, pre-tonemap
        outputColor = renodx::color::grade::UserColorGrading(outputColor,
    ...
    if (userToneMapType != 0) {  // UserColorGrading, post-tonemap
        outputColor =
            renodx::color::grade::UserColorGrading(outputColor,
                                                   1.f,                             // exposure
                                                   1.f,                             // highlights
                                                   1.f,                             // shadows
                                                   1.f,                             // contrast
...
float3 UpgradeToneMapCustom(float3 color_untonemapped, float3 color_tonemapped, float3 color_tonemapped_graded,
                            float post_process_strength = 1.f, float auto_correction = 0.f) {
    float ratio = 1.f;
...
float3 ToneMapPassCustom(float3 color, float ignore_tonemap_flag = 0.f) {
    ...
    /*
      if (RENODX_DEBUG_MODE3 == 2.0f) {
        ...
        color = UpgradeToneMapCustom(untonemapped, neutral_sdr_color, color, shader_injection.color_grade_strength,
      0.f);
      }*/
...
float3 ToneMapPassCustom(float3 untonemapped, float3 graded_sdr_color) {
    float3 color =
        ComputeUntonemappedGraded(untonemapped, graded_sdr_color, renodx::tonemap::renodrt::NeutralSDR(untonemapped));
