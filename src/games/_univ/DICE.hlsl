...
float luminanceCompress(float InValue, float OutMaxValue, float ShoulderStart = 0.f, bool considerMaxValue = false,
    ...
    const float possibleOutValue =
        ...
        + compressedRange
              * rangeCompress(compressableValue / compressedRange,
                              considerMaxValue ? (compressableRange / compressedRange) : renodx::math::FLT_MAX);
#if 1
...
// Doing the DICE compression in PQ (either on luminance or each color channel) produces a curve that is closer to our
// "perception" and leaves more detail highlights without overly compressing them
#define DICE_TYPE_BY_LUMINANCE_PQ 1
// Modern HDR displays clip individual rgb channels beyond their "white" peak brightness,
// like, if the peak brightness is 700 nits, any r g b color beyond a value of 700/80 will be clipped (not acknowledged,
// it won't make a difference). Tonemapping by luminance, is generally more perception accurate but can then generate
// rgb colors "out of range". This setting fixes them up, though it's optional as it's working based on assumptions on
// how current displays work, which might not be true anymore in the future. Note that this can create some steep
// (rough, quickly changing) gradients on very bright colors.
#define DICE_TYPE_BY_LUMINANCE_PQ_CORRECT_CHANNELS_BEYOND_PEAK_WHITE 2
// This might look more like classic SDR tonemappers and is closer to how modern TVs and Monitors play back colors
// (usually they clip each individual channel to the peak brightness value, though in their native panel color space, or
// current SDR/HDR mode color space). Overall, this seems to handle bright gradients more smoothly, even if it shifts
// hues more (and generally desaturating).
...
float3 DICETonemap(float3 Color, float PeakWhite, const DICESettings Settings) {
    ...
    if (Settings.Type != DICE_TYPE_BY_LUMINANCE_RGB) {
        ...
        if (Settings.Type == DICE_TYPE_BY_LUMINANCE_PQ
            ...
            if (sourceLuminancePQ
                > shoulderStartPQ)  // Luminance below the shoulder (or below zero) don't need to be adjusted
            {
                ...
                if (Settings.Type == DICE_TYPE_BY_LUMINANCE_PQ_CORRECT_CHANNELS_BEYOND_PEAK_WHITE) {
                    ...
                    if (any(Color_BT2020 > PeakWhite))  // Optional "optimization" branch
                    {
                        ...
                        float maxColorInExcess =
                            max3(Color_BT2020) - PeakWhite;  // This is guaranteed to be >= "colorLuminanceInExcess"
                        float brightnessReduction = saturate(renodx::math::SafeDivision(
                            PeakWhite, max3(Color_BT2020), 1));  // Fall back to one in case of division by zero
                        float desaturateAlpha = saturate(
                            renodx::math::SafeDivision(maxColorInExcess, maxColorInExcess - colorLuminanceInExcess,
                                                       0));  // Fall back to zero in case of division by zero
                        Color_BT2020 =
                            lerp(Color_BT2020, colorLuminance, desaturateAlpha * Settings.DesaturationAmount);
                        Color_BT2020 = lerp(Color_BT2020, Color_BT2020 * brightnessReduction,
                                            Settings.DarkeningAmount);  // Also reduce the brightness to partially
                                                                        // maintain the hue, at the cost of brightness
        ...
        } else  // DICE_TYPE_BY_CHANNEL_PQ
        {
            ...
            for (uint i = 0; i < 3;
                 i++)  // TODO LUMA: optimize? will the shader compile already convert this to float3? Or should we
                       // already make a version with no branches that works in float3?
            {
                if (sourceColorPQ[i]
                    > shoulderStartPQ)  // Colors below the shoulder (or below zero) don't need to be adjusted
                {
    ...
    {
        if (sourceLuminance
            > Settings.ShoulderStart)  // Luminance below the shoulder (or below zero) don't need to be adjusted
        {
            const float compressedLuminance =
                luminanceCompress(sourceLuminance, PeakWhite, PeakWhite * Settings.ShoulderStart);
