...
float3 UnityLookupARID(float4 input_color, float4 lut_param, SamplerState s0_s, Texture2D<float4> lut) {
    ...
    r0.xyz = log2(r0.xyz);
    r0.xyz = saturate(r0.xyz * float3(0.0734997839, 0.0734997839, 0.0734997839)
...
float4 UnityYRGBCurvesLutBuilder(float3 input_color, SamplerState texSampler, Texture2D<float4> lut_y,
                                 Texture2D<float4> lut_r, Texture2D<float4> lut_g, Texture2D<float4> lut_b,
                                 float4 cb0_5) {
    float3 untonemappedSign = sign(input_color);
