...
SUBSTRING_TYPES = {
    ...
    'lutbuilder': [
        # Example: both 'LUT' and 'builder' must be present
        ["0.00390625", "0.549941003"],
...
def convert_cso_to_hlsl(cso_file_path):
    ...
    try:
        ...
        result = subprocess.run(cmd, capture_output=True,
                                text=True, cwd=SCAN_DIR)

        if result.returncode == 0:
            print(
                f"Successfully converted {os.path.basename(cso_file_path)} to HLSL")
            return True
        else:
            print(
                f"Failed to convert {os.path.basename(cso_file_path)}: {result.stderr}")
            return False
    ...
    for fname in os.listdir(SCAN_DIR):
        if fname.lower().endswith('.cso'):
            ...
            if not os.path.exists(hlsl_path):
                ...
                print(
                    f"HLSL file {hlsl_name} already exists, skipping conversion")
    ...
    for fname in os.listdir(SCAN_DIR):
        ...
        if file_matches_type(src_path, type_key):
            ...
            if match:
                ...
                cand_files = os.listdir(
                    cand_dir) if os.path.exists(cand_dir) else []

                # Check subfolders
                ubers_dir = os.path.join(DEST_DIR, "ubers")
                lutbuilders_dir = os.path.join(DEST_DIR, "lutbuilders")
                ubers_files = os.listdir(
                    ubers_dir) if os.path.exists(ubers_dir) else []
                lutbuilders_files = os.listdir(
                    lutbuilders_dir) if os.path.exists(lutbuilders_dir) else []
                ...
                if found:
                    print(
                        f"A file with hex pattern {hex_pattern} already exists in destination or candidate directory, skipping {fname}. Full path: {src_path}")
                    continue
                ...
                print(
                    f"Moving {fname} -> {os.path.relpath(cand_path, DEST_DIR)}")
                ...
                if os.path.exists(cso_src_path):
                    cso_cand_path = os.path.join(
                        cand_dir, cso_name.replace('.cso', f'_{hex_part}.cso'))
                    print(
                        f"Moving {cso_name} -> {os.path.relpath(cso_cand_path, DEST_DIR)}")
