...
float3 ApplyPerceptualBoostAndToneMap(float3 color, float scene_type = SCENE_TYPE_UNKNOWN, float2 uv = float2(0.5, 0.5)) {

    if (uv.x < shader_injection.horizontal_split_screen) {
        return color;
    }
...
float3 ApplyPerceptualBoostAndToneMapSRGB(float3 color, float scene_type = SCENE_TYPE_UNKNOWN) {
