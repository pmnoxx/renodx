...
void AddReflexSettings(std::vector<renodx::utils::settings::Setting*>& settings) {
    ...
    settings.push_back(new renodx::utils::settings::Setting{
        ...
        .is_visible = []() { return is_basic_tab(s_ui_mode) && s_reflex_enabled >= 0.5f; }, // Show when Reflex is enabled
    });
    ...
    settings.push_back(new renodx::utils::settings::Setting{
        ...
        .labels = {"Disabled", "Enabled"},
        .is_visible = []() { return is_developer_tab(s_ui_mode) && s_reflex_enabled >= 0.5f; }, // Keep in Developer mode
    });
}
