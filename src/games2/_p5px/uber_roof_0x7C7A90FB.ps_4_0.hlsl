...
// ---- Created with 3Dmigoto v1.4.1 on Thu Jun 26 06:19:42 2025
  ...
  out float4 o0 : SV_Target0)
{
 ...
 // untonemapped.xyz *= 10.f;

  if (RENODX_TONE_MAP_TYPE > 0.f) {
    //float3 lut_input = renodx::color::arri::logc::c1000::Encode(untonemapped, false);
    float3 lut_input = renodx::color::pq::Encode(untonemapped, 100.f);
    r0.xyz = renodx::lut::Sample(t2, s2_s, saturate(lut_input), cb0[39].yzw);

 /*   r0.xyz= abs(saturate(lut_input) -  r0.xyz);
    o0.w = 1.f;
    return;*/
  } else {
